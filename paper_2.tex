...
IPv6 addresses the shortage in the short term by redefining the unicast and anycast address format. Unicast is a peer-too-peer connection, while anycast is a multi-relay connection where multiple devices connect to a common address, as is the case with printers. All IPv6 address are composed of two unique logcal identifiers: a 64-bit network prefix which routes the traffic, and a 64-bit interface identifier used to identify the network interface (i.e eth0) of the host. The size of the routing prefix varies, with larger hex values signifying a sub-net configuration such as those found on university and corporate networks. Sub-net ID parameter bits are available to be modified by the network administrator or can be set through a DHCPv6 server (DHCP servers allow for dynamic IP address allocation). DHCPv4 will forward all address to a previously-discussed NAT system unless another translation broadcast has been sent to clients. The interface identifier is also obtained from a DHCPv6 server \cite{rfc8064}.

As IPv6 adoption rises, similar challenges are predicted, although its larger address space may delay the issue for some time. This will also lead to increased overhead in address aggregation and routing complexity \cite{liu2010incremental}. 
